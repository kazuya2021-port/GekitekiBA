...
@class AcrobatProApplication, AcrobatProDocument, AcrobatProWindow, AcrobatProPDFWindow, AcrobatProPage, AcrobatProBookmark, AcrobatProAnnotation, AcrobatProTextAnnotation, AcrobatProLinkAnnotation, AcrobatProConversion, AcrobatProPostScriptConversion, AcrobatProEPSConversion, AcrobatProMenu, AcrobatProMenuItem;
...
enum AcrobatProBkft {
	AcrobatProBkftLeftTopZoom = 'ftlz' /* Fit to top left and zoom */,
	AcrobatProBkftFitPage = 'fpag' /* Fit page */,
	AcrobatProBkftFitWidth = 'fwid' /* Fit width */,
	AcrobatProBkftFitHeight = 'fhei' /* Fit height */,
	AcrobatProBkftFitRect = 'frec' /* Fit rect */,
	AcrobatProBkftFitBbox = 'fbbo' /* Fit bounding box */,
	AcrobatProBkftFitBbwidth = 'fbbw' /* Fit bounding box width */,
	AcrobatProBkftFitBbheight = 'fbbh' /* Fit bounding box height */
};
typedef enum AcrobatProBkft AcrobatProBkft;
...
@protocol AcrobatProGenericMethods

- (void) closeSaving:(AcrobatProSavo)saving linearize:(BOOL)linearize;  // Close a document.
...
- (void) selectTextFromWords:(NSArray<NSNumber *> *)fromWords fromChars:(NSArray<NSNumber *> *)fromChars;  // Selects text as specified by either character or word offsets.
...
- (SBElementArray<AcrobatProDocument *> *) documents;
- (SBElementArray<AcrobatProPDFWindow *> *) PDFWindows;
- (SBElementArray<AcrobatProMenu *> *) menus;
- (SBElementArray<AcrobatProMenuItem *> *) menuItems;
- (SBElementArray<AcrobatProConversion *> *) conversions;
...
@property AcrobatProPdzt defaultZoomType;  // The default zoom type when opening a new document. Valid values are “no vary”, “fit page”, “fit width”, “fit height”, and “fit visible width”.
...
@property AcrobatProPfcd fullscreenCursor;  // Cursor visibility in fullscreen mode. Valid values are “always visible”, “always hidden” or “hidden after delay”).
...
@property (copy) NSColor *highlightColor;  // Color used to highlight selections.
...
@property (copy) NSColor *noteColor;  // The color of the border around newly created text annotations.
...
@property (copy, readonly) NSString *UILanguage;  // Identifies which language Acrobat’s UI is using. This is a 3 character language codes (ENU is English, for instance).
@property (copy, readonly) NSString *version;  // The version number of the application.
@property BOOL wholeWordSearching;  // Whether searches are for whole words.

- (void) closeSaving:(AcrobatProSavo)saving linearize:(BOOL)linearize;  // Close a document.
- (void) open:(NSArray<NSURL *> *)x invisible:(BOOL)invisible options:(NSString *)options;  // Open a document or documents.
- (void) quitSaving:(AcrobatProSavo)saving;  // Tell Acrobat to quit.
- (void) clearSelection;  // Clear any document selection.
- (void) closeAllDocsSaving:(AcrobatProSavo)saving;  // Close all documents, with optional saving first.
- (void) createThumbs;  // Create thumbnails for all pages in the document.
- (void) deletePagesFirst:(NSInteger)first last:(NSInteger)last;  // Deletes the specified pages in the document.
- (void) deleteThumbs;  // Delete all thumbnails from the document.
- (SBObject *) findNextNoteWrapAround:(BOOL)wrapAround;  // Find and select the next text note.
...
// An Acrobat document.
@interface AcrobatProDocument : SBObject <AcrobatProGenericMethods>

- (SBElementArray<AcrobatProPage *> *) pages;
- (SBElementArray<AcrobatProBookmark *> *) bookmarks;
- (SBElementArray<AcrobatProPDFWindow *> *) PDFWindows;
...
@property AcrobatProPvew viewMode;  // The view mode of the document (one of: “just pages”, “pages and thumbs”, or “pages and bookmarks”).


@end

// A window.
@interface AcrobatProWindow : SBObject <AcrobatProGenericMethods>
...
// The window displaying the specified document
...
@property AcrobatProPdzt zoomType;  // The zooming and content fitting algorithm current employed. Valid values are “no vary”, “fit page”, “fit width”, “fit height”, and “fit visible width”.


@end

// A document page.
@interface AcrobatProPage : SBObject <AcrobatProGenericMethods>

- (SBElementArray<AcrobatProAnnotation *> *) annotations;

@property (copy, readonly) NSNumber *bestType;  // The best descriptor type.
@property (copy) NSArray<NSNumber *> *artBox;  // The art box rectangle for the page, in PDF space (left, top, right, bottom).
@property (copy) NSArray<NSNumber *> *bleedBox;  // The bleed box rectangle for the page, in PDF space (left, top, right, bottom).
@property (copy) NSArray<NSNumber *> *cropBox;  // The crop rectangle for the page, in PDF space (left, top, right, bottom).
@property (copy, readonly) NSNumber *defaultType;  // The default descriptor type.
@property (copy, readonly) NSString *labelText;  // The label (or custom page number) used to describe the page.
@property (copy) NSArray<NSNumber *> *mediaBox;  // The media bounds rectangle for the page, in PDF space (left, top, right, bottom).
...
@property (copy) NSArray<NSNumber *> *trimBox;  // The trim box rectangle for the page, in PDF space (left, top, right, bottom).


@end

// A bookmark.
@interface AcrobatProBookmark : SBObject <AcrobatProGenericMethods>

@property (copy, readonly) NSNumber *bestType;  // The best descriptor type.
@property (copy, readonly) NSNumber *defaultType;  // The default descriptor type.
@property NSInteger destinationPageNumber;  // The number of the page the PDF Window goes to when the bookmark is performed.
@property (copy) NSArray<NSNumber *> *destinationRectangle;  // The boundary rectangle for the view of the destination, in PDF space (left, top, right, bottom). [Set this only after setting the fit type property].
@property AcrobatProBkft fitType;  // Controls how the destination rectangle is fitted to the window when the bookmark is executed. Valid values are: “Left Top Zoom”, “Fit Page”, “Fit Width”, “Fit Height”, “Fit Rect”, “Fit BBox”, “Fit BB Width”, and “Fit BB Height”.
...
// An annotation.
@interface AcrobatProAnnotation : SBObject <AcrobatProGenericMethods>

@property (copy, readonly) NSNumber *bestType;  // The best descriptor type.
@property (copy) NSArray<NSNumber *> *bounds;  // The boundary rectangle for the annotation, in PDF space (left, top, right, bottom).
@property (copy) NSColor *color;  // The color of the border around the annotation.
...
@property (copy) NSArray<NSNumber *> *destinationRectangle;  // Link subtype only: The boundary rectangle for the view of the destination, in PDF space (left, top, right, bottom).
@property AcrobatProBkft fitType;  // Link subtype only: Controls how the destination rectangle is fitted to the window when the annotation is performed. Valid values are: “Left Top Zoom”, “Fit Page”, “Fit Width”, “Fit Height”, “Fit Rect”, “Fit BBox”, “Fit BB Width”, and “Fit BB Height”.
...
@interface AcrobatProConversion : SBObject <AcrobatProGenericMethods>
...
@interface AcrobatProMenu : SBObject <AcrobatProGenericMethods>
...
// A menu item.
@interface AcrobatProMenuItem : SBObject <AcrobatProGenericMethods>
